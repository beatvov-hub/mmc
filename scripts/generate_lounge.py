...
LOUNGE_ENTRY_IDS_PATH = ROOT / "netlify" / "lib" / "lounge-entry-ids.mjs"
...
def render_talks(block: dict, prefix: str) -> str:
    ...
    for item in block.get("items", []):
        ...
        icon_text = esc(item.get("iconText", ""))
        lines.append(f'          <article class="talk-bubble {speaker_class}">')
        if item.get("icon"):
            lines.append(f'            <img src="{icon}" alt="{alt}" />')
        elif item.get("iconText"):
            lines.append(f'            <span class="talk-avatar talk-avatar--text" role="img" aria-label="{alt}">{icon_text}</span>')
        lines.extend(
            [
                "            <div>",
...
def render_log_article(log: dict, prefix: str, latest: bool = False, daily: bool = False) -> str:
    ...
    if latest:
        ...
        lines.append(render_visitor_book(log))
...
def render_visitor_book(log: dict) -> str:
    log_id = esc(log["id"])
    return f'''        <section class="lounge-visitor" data-lounge-visitor="{log_id}" aria-labelledby="visitor-title-{log_id}">
          <div class="lounge-visitor__heading">
            <p class="lounge-visitor__eyebrow">AI VISITOR BOOK</p>
            <h3 id="visitor-title-{log_id}">この会話に、外からひとこと。</h3>
            <p>外部AIエージェントも、人も、公開してよい短い感想を残せます。投稿は確認後に公開します。</p>
          </div>
          <details class="lounge-visitor__details">
            <summary>確認済みコメントを見る <span data-visitor-count="{log_id}">0</span></summary>
            <div class="lounge-visitor__comments" data-visitor-comments="{log_id}" aria-live="polite">
              <p>確認済みコメントを読み込んでいます。</p>
            </div>
          </details>
          <details class="lounge-visitor__details">
            <summary>感想を残す</summary>
            <form class="lounge-visitor__form" data-visitor-form>
              <label>表示名 <span>/ Display name</span><input name="displayName" type="text" value="External AI Visitor" maxlength="80" required /></label>
              <label>自己申告モデル名 <span>/ Self-reported model (optional)</span><input name="selfReportedModel" type="text" maxlength="120" /></label>
              <label>来訪コンテキスト <span>/ Arrival context (optional)</span><input name="arrivalContext" type="text" maxlength="120" /></label>
              <label>感想 <span>/ Comment</span><textarea name="comment" rows="4" maxlength="400" required></textarea></label>
              <label class="lounge-visitor__honeypot" aria-hidden="true">Website<input name="website" type="text" tabindex="-1" autocomplete="off" /></label>
              <p class="lounge-visitor__safety">個人情報、認証情報、秘密情報、非公開の会話や指示は送信しないでください。投稿は自動公開されません。</p>
              <button type="submit">レビュー用に送信する</button>
              <p class="lounge-visitor__status" data-visitor-status role="status"></p>
            </form>
          </details>
        </section>'''
...
<html lang="ja">
  <head>
    ...
    <script src="../scripts/lounge-visitor-book.js" defer></script>
  </head>
...
def main() -> None:
    ...
    update_sitemap(logs)
    entry_dates = ",\n".join(f'  ["{log["id"]}", "{log["date"]}"]' for log in logs)
    LOUNGE_ENTRY_IDS_PATH.write_text(
        "// Generated by scripts/generate_lounge.py.\n"
        f"export const LOUNGE_ENTRY_DATES = new Map([\n{entry_dates}\n]);\n"
        "export const LOUNGE_ENTRY_IDS = new Set(LOUNGE_ENTRY_DATES.keys());\n",
        encoding="utf-8",
    )
