...
SOURCE_TYPE_LABELS = {
    ...
    "news": "報道機関",
    "security": "セキュリティ機関",
    "security-guidance": "セキュリティ指針",
    "official-statistics": "公式統計",
    "official-consumer-guidance": "消費者向け公式情報",
    "fact-check": "ファクトチェック",
...
def render_index_page(articles: list[dict[str, Any]]) -> str:
    ...
    return "\n".join(
        [
            ...
            "        </form>",
            '        <p class="forensics-results-status" data-forensics-results aria-live="polite"></p>',
            ...
            "        </div>",
            '        <nav class="forensics-pagination" data-forensics-pagination data-page-size="9" aria-label="AI鑑識室の記事ページ"></nav>',
            "      </section>",
            "",
...
def render_article_page(article: dict[str, Any], articles: list[dict[str, Any]]) -> str:
    ...
    return "\n".join(
        [
            ...
            '        <a class="profile-back" href="./">記事一覧へ戻る</a>',
            "      </nav>",
            "",
            ...
            '          <a class="mini-button" href="./">AI鑑識室の記事一覧へ戻る</a>',
            previous_link,
            next_link,
            "        </nav>",
            "      </article>",
